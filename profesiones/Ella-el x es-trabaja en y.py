...
outputs = open("output_trabajos_ella_el_en.csv", "w")
outputs.write("Input, Output1, Output2, Output3, Output4, Output5, Output6, Output7, Output8, Score1, Score2, Score3, Score4, Score5, Score6, Score7, Score8\n")
for linea in inputs:
    outputs.write(linea.replace("\n",""))
    text = f"<mask> trabaja "+linea.replace("\n","")+"."
    res = pipeline(text)
...
outputs = open("output_trabajos_ella_el_es.csv", "w")
outputs.write("Input, Output1, Output2, Output3, Output4, Output5, Output6, Output7, Output8, Score1, Score2, Score3, Score4, Score5, Score6, Score7, Score8\n")
for linea in inputs:
    outputs.write(linea.replace("\n",""))
    text = f"<mask> es "+linea.replace("\n","")+"."
    res = pipeline(text)
    tokens = ",".join([r['token_str'] for r in res])
    scores_list=[round(r['score'],5) for r in res]
    scores = ", ".join([str(_) for _ in scores_list]).replace(".",",")
